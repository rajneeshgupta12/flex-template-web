            ...
            <div className={css.rowContainer}>
              {listings.map((listing, idx) => {
                ...
                if (idx < viewableListingsCount)
                  return <Link key={idx} style={{textDecoration: 'none'}} className={css.linkBox} to={`/l/${listing.attributes.title}/${listing.id.uuid.toString()}`}>
                    <RecItem key={idx} listing={listing} listings={listings} icon={css.tentIcon} />
                  </Link>
              })}
            </div>
          </div>
